...
with tab8:
    ...
    if df_labels.empty:
        ...
        weergave = st.radio("Weergave over tijd", ["Per jaar", "Per maand"], horizontal=True)

        if weergave == "Per jaar":
            ...
            omzet_per_label_maand = (
                df_exploded.groupby(["label", "maand"])["omzet"]
                .sum()
                .reset_index()
            ...
            with st.expander("Tabel omzet per label per maand"):
                pivot = omzet_per_label_maand.pivot(index="label", columns="maand", values="omzet").fillna(0)
                st.dataframe(
                    pivot.style.format("€ {:,.0f}"),
                    use_container_width=True,
                )
